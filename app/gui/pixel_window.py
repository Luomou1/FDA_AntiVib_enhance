...
class PixelAnalysisWindow(QMainWindow):
    ...
    def update_analysis(self, payload: dict, layer_name: str, fitting_method: str, unwrap_method: str) -> None:
        ...
        self.raw_axes.set_title("Coherence Profile", fontproperties=FONT_PROP)
        self.raw_axes.set_xlabel("\u626b\u63cf\u4f4d\u7f6e z (um)", fontproperties=FONT_PROP)
        self.raw_axes.set_ylabel("Intensity (a.u.)", fontproperties=FONT_PROP)

        self.dc_axes.clear()
        self.dc_axes.plot(payload["signal_x"], payload["signal_dc_y"], color="#d94841", linewidth=1.2)
        self.dc_axes.set_title("Coherence Profile (DC Removed)", fontproperties=FONT_PROP)
        self.dc_axes.set_xlabel("\u626b\u63cf\u4f4d\u7f6e z (um)", fontproperties=FONT_PROP)
        self.dc_axes.set_ylabel("Intensity (a.u.)", fontproperties=FONT_PROP)
        ...
        self.amp_axes.set_title("Amplitude Spectrum", fontproperties=FONT_PROP)
        self.amp_axes.set_xlabel("\u6ce2\u6570 k (rad/um)", fontproperties=FONT_PROP)
        self.amp_axes.set_ylabel("Relative Amplitude (a.u.)", fontproperties=FONT_PROP)

        self.phase_raw_axes.clear()
        self.phase_raw_axes.plot(payload["k_x"], phase_raw_cycles, color="#9467bd", linewidth=1.2)
        self.phase_raw_axes.set_title("Phase Spectrum", fontproperties=FONT_PROP)
        self.phase_raw_axes.set_xlabel("\u6ce2\u6570 k (rad/um)", fontproperties=FONT_PROP)
        self.phase_raw_axes.set_ylabel("Phase (cycles)", fontproperties=FONT_PROP)
        ...
        if unwrap_method == "global":
            ...
            if fit_k_x is not None and fit_phase_y is not None and len(fit_k_x) >= 2:
                ...
                k_min = float(np.min(fit_k_x))
                k_max = float(np.max(fit_k_x))
                k_span = max(1e-6, k_max - k_min)
                self.phase_unwrapped_axes.set_xlim(k_min - 0.8 * k_span, k_max + 0.8 * k_span)
        ...
        self.phase_unwrapped_axes.set_title("Phase Profile", fontproperties=FONT_PROP)
        self.phase_unwrapped_axes.set_xlabel("\u6ce2\u6570 k (rad/um)", fontproperties=FONT_PROP)
        self.phase_unwrapped_axes.set_ylabel("Phase (cycles)", fontproperties=FONT_PROP)
